...
`SYN` and `DV` links are checked one by one against the current checked-in file in each IP. When an IP publishes a master `SIGNOFF.md`, both columns intentionally point to that master dashboard. `—` means no synthesis or verification signoff document is currently indexed from this top-level catalog.
