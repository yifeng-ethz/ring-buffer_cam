...
- [x] `mutrig_timestamp_processor/mts_processor.vhd`: replace the fixed delayed global terminate pulse with lane-targeted close markers on the `hit_type1` sideband channel.
- [x] `mutrig_timestamp_processor/tb/mts_processor_terminating_tb.vhd`: add a directed case that expects one empty close marker per downstream lane after payload drain.
- [x] `mutrig_timestamp_processor/tb/uvm/*`: add or update a terminate-contract case so the scoreboard checks lane-targeted empty close markers and late ready ack.
- [x] `ring-buffer_cam/ring_buffer_cam_v2_core.vhd`: consume lane-targeted empty close markers without writing them into the deassembly FIFO; latch end-of-run only for the local interleaving lane.
- [x] `ring-buffer_cam/ring_buffer_cam.vhd`: expose the new `asi_hit_type1_empty` ingress sideband through the RTL wrapper. `ring_buffer_cam_hw.tcl` is intentionally deferred.
- [x] `ring-buffer_cam/tb/sim/*`: add a lane-local directed terminate case that proves an empty close marker drains the instance and does not create a ghost hit. `tb/uvm/*` is updated for the new `empty` sideband and rerun with the existing smoke test.
- [x] `packet_scheduler/tb_int/rtl/datapath_stub.sv`: wire the MTS empty-close sideband into each `ring_buffer_cam` instance.
- [x] `packet_scheduler/tb_int/uvm/tb_int_pkg.sv`: stop counting empty close markers as H1 hits while still auditing their lane/EOP timing.
- [x] `packet_scheduler/tb_int/uvm/*`: add an integration testcase that keeps the current FEB-output accounting intact while exercising the upgraded terminate/drain path. The full end-to-end run is still too slow for interactive closure here, but the testcase is added and the build compiles.
