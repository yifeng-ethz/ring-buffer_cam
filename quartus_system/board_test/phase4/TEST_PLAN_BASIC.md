...
**Companion:** [`test_plan.md`](test_plan.md) · [`MATH_REPORT.md`](MATH_REPORT.md) · [`TLM_REPORT.md`](TLM_REPORT.md) · [`TEST_REPORT.md`](TEST_REPORT.md) · [`../doc/TEST_PLAN.md`](../doc/TEST_PLAN.md) · [`../../model/`](../../model/)
...
**Upstream art-of-truth:** the MuTRiG delay-distribution slides at [`../../../doc/Archive/ethhw_reordering.pdf`](../../../doc/Archive/ethhw_reordering.pdf) (slide 23 random injection, slide 24 frame-synchronous header-delay sweep, slide 25 rate sweep, slide 38 ingress/egress comparison, slides 40–42 ring-CAM fill-level). The TLM model is calibrated to match these distributions; if SIM or board disagrees with TLM, debug starts at SIM/RTL — never edit the TLM to match a buggy SIM.

## Method / implementation legend

- **method** — `D` = directed (one deterministic stimulus configuration per case) · `R` = random (multi-seed)
- **implementation** — `live TLM` = Python/HDL TLM in [`scripts/`](scripts/) and [`tlm/`](tlm/) · `live SIM` = RTL integration sim driven by [`../../tb_int/INT_fe_scifi_v3-2026-04-17/scripts/run_dp_e2e.sh`](../../tb_int/INT_fe_scifi_v3-2026-04-17/scripts/run_dp_e2e.sh) and [`../../tb_int/INT_fe_scifi_v3-2026-04-17/scripts/run_dp_hist_rate_sweep.sh`](../../tb_int/INT_fe_scifi_v3-2026-04-17/scripts/run_dp_hist_rate_sweep.sh) · `live BOARD` = on-board run via [`../script/run_phase4_emulator.py`](../script/run_phase4_emulator.py) and [`../script/probe_phase4_stage_counters.py`](../script/probe_phase4_stage_counters.py) · `live CMP` = DISLIN cross-layer compare via [`scripts/render_phase4_plots.sh`](scripts/render_phase4_plots.sh) · `planned` = stimulus exists, runner not yet wired
...
|---|---|---|---|
| 8 × `emulator_mutrig_N` (N=0..7) | `lvds_rx_28nm_0.outclock`, `master_datapath.master_reset` | per-lane `aso_tx8b1k` into hit_stack subsystem | `emulator_mutrig_N.csr[0x05]` `frame_count` increments (per `../doc/TEST_PLAN.md` §4.2) |
...
| 7 × `hist_rate_splitter_N.out1 → histogram_statistics_0.fill_in_N` and `histogram_ingress_bridge_0.hist_out → histogram_statistics_0.hist_fill_in` | `scifi_datapath_system_v3.qsys` lines 2353, 2365–2395 | `histogram_statistics_0.csr[0x34]` `TOTAL_HITS` advances together with the SWB per-link counter | `../doc/TEST_PLAN.md` §4.4 |
...
|---|---|---|---|---|---|
| TPB000 | D | live BOARD | preflight `../doc/TEST_PLAN.md` §0 + §4.1 — flash SWB from `online_sc`, FEB SciFi v3 from this repo, recover `/dev/mudaq0`, `script/build_local_tools.py`, `sc_tool read 0x00000` returns OK, `histogram_statistics_0.UID = 0x48495354` | every step in §0 passes; bridged read of `histogram_statistics_0` UID succeeds; emulator+injector instances are reachable through `mm_bridge` | B |
...
| TPB003 | D | live CMP | DISLIN render via [`scripts/render_phase4_plots.sh`](scripts/render_phase4_plots.sh) produces the four plots required by [`test_plan.md`](test_plan.md) §6 | DISLIN log reports `Warnings: 0`; PNG and SVG present for latency, rate, queue depth, MuTRiG injection | C |
...
| TPB053 | D | live BOARD | inject mode 1 with `INJ_ARM` SignalTap trigger from `../doc/TEST_PLAN.md` §4.3 (injector armed outside RUNNING) | trigger never fires during the test window; if it does, the test is invalid | B |
...
| TPB074 | D | live BOARD | repeat TPB073 for 60 s soak with `--measure-after-end` post-end flush check | post-end flush clean; `DROPPED_HITS delta=0` (matches `test_plan.md` Pass Criteria) | B |
...
| TPB083 | D | live BOARD | refreshed board run via `script/run_phase4_emulator.py --measure-after-end --output reports/phase4_emulator_<date>_target200.md` (per `test_plan.md` step 7) | board run reports the 200 Mhit/s clip target, not the legacy 135 Mhit/s knee; `DROPPED_HITS=0`, post-end flush clean | B |
...
| TPB091 | D | live BOARD | `BANK_STATUS` ping-pong sanity over 32 intervals | bank alternates with stddev < 1 interval (matches `../doc/TEST_PLAN.md` §4.5.4) | B |
| TPB092 | D | live BOARD | SignalTap one-shot capture for `OVRFL_INC` and `UNDR_INC` triggers (per `../doc/TEST_PLAN.md` §4.3) over 60 s clean run | neither trigger fires; if either does, halt and route to TPBH series before further TPB cases | B |
...
| TPB098 | D | live BOARD | full-image regression: TPB000..TPB096 across the latest FEB SciFi v3 SOF and the latest SWB `online_sc` SOF | every case PASS; the `phase4_emulator_<date>.md` and `phase4_stage_probe_<date>.md` reports are checked into `board_test/reports/` | B |
...
| TPB100 | D | live BOARD, live CMP | Phase 4 closure attestation: TPBH series (below) PASS + TPB000..TPB099 PASS + TPB003 plot bundle PASS + cross-layer disagreement ratios all within tolerance | `test_plan.md` "Closure" section is signed; no Phase 4 closure without this case PASS | B/C |
...
| TPBH029 | D | live BOARD | full pre-gate signoff run: TPBH018..TPBH023 chained, single bucket-frame, single report | one report file in `board_test/reports/phase4_overflow_pregate_<date>.md`; ALL deltas zero | B |
