...
entity ring_buffer_cam_legacy_core is 
generic(
...
end entity ring_buffer_cam_legacy_core;

architecture rtl of ring_buffer_cam_legacy_core is 
	function rotate_match_vector_by_write_pointer (
		raw_match_vec : std_logic_vector;
		base_ptr      : unsigned
	) return std_logic_vector is
		variable rotated_v  : std_logic_vector(raw_match_vec'range);
		variable phys_idx_v : natural;
		variable base_nat_v : natural;
	begin
		base_nat_v := to_integer(base_ptr);
		for age_idx in 0 to raw_match_vec'length-1 loop
			phys_idx_v := age_idx + base_nat_v;
			if (phys_idx_v >= raw_match_vec'length) then
				phys_idx_v := phys_idx_v - raw_match_vec'length;
			end if;
			rotated_v(age_idx) := raw_match_vec(phys_idx_v);
		end loop;
		return rotated_v;
	end function;

	function age_order_idx_to_physical_idx (
		age_idx  : natural;
		base_ptr : unsigned
	) return natural is
		variable phys_idx_v : natural;
	begin
		phys_idx_v := age_idx + to_integer(base_ptr);
		if (phys_idx_v >= RING_BUFFER_N_ENTRY) then
			phys_idx_v := phys_idx_v - RING_BUFFER_N_ENTRY;
		end if;
		return phys_idx_v;
	end function;
    ...
    constant TERMINATION_DRAIN_GUARD_CYCLES_CONST : natural := (INTERLEAVING_FACTOR - 1) * (2**SK_RANGE_LO);
	...
	signal cam_match_addr_oh_age_order_comb : std_logic_vector(MAIN_CAM_SIZE-1 downto 0);
	...
	-- Safe address conversion for port maps (avoid to_integer of metavalues)
	signal side_ram_raddr_nat				: natural := 0;
	signal side_ram_waddr_nat				: natural := 0;
...
begin
	...
	side_ram_raddr_nat <= safe_to_nat(side_ram_raddr);
	side_ram_waddr_nat <= safe_to_nat(side_ram_waddr);
...
--						debug_msg.overwrite_cnt		<= (others => '0');
	...
	begin
		if (i_rst = '1') then 
            ...
            -- Keep one extra interleaving round alive so FEB can continue
            -- merging later timestamps after a lane has already moved past EOR.
				if (read_time_ptr <= gts_end_of_run + to_unsigned(TERMINATION_DRAIN_GUARD_CYCLES_CONST, gts_end_of_run'length)) then
					if (read_time_ptr(3 downto 0) = "0000" and to_integer(unsigned(read_time_ptr(TCC8N_INTERLEAVING_BITS+3 downto 4))) = INTERLEAVING_INDEX ) then -- generate read command every 16 cycle 
						pop_cmd_fifo_wrreq		<= '1';
						pop_cmd_fifo_din		<= std_logic_vector(read_time_ptr(SK_RANGE_HI+1 downto SK_RANGE_LO)); -- NOTE: search key also controls the subheader gen cmd
					end if;
...
--						debug_msg.pop_cnt			<= debug_msg.pop_cnt + 1;	-- incr global counter
				...
				when FLUSHING_RST =>
					...
					pop_flush_ram_done				<= '0';
					pop_flush_cam_done				<= '0';
					-- flush counters (reset for next flush cycle)
					flush_ram_wraddr				<= (others => '0');
					flush_cam_wraddr				<= (others => '0');
					flush_cam_wrdata				<= (others => '0');
					-- pointer 
...
--if (to_integer(unsigned(bank_combiner_total_count_comb)) /= 0) then 
		...
		cam_match_addr_oh_age_order_comb <= rotate_match_vector_by_write_pointer(
			cam_match_addr_oh,
			write_pointer
		);
		-- transformation age-ordered cam_match_addr_oh to 2d
		for i in 0 to N_SMALL_LOGIC-1 loop
			cam_match_addr_oh_2d_comb(i)		<= cam_match_addr_oh_age_order_comb((i+1)*SMALL_LOGIC_CAM_SIZE-1 downto i*SMALL_LOGIC_CAM_SIZE);
		end loop;
	...
	begin
		...
		for i in 0 to N_SMALL_LOGIC-1 loop
			if (bank_combiner_grant_comb(i) = '1') then -- grant this way
				pop_cam_match_addr			<= std_logic_vector(to_unsigned(
					age_order_idx_to_physical_idx(
						to_integer(unsigned(bank_combiner_addr0(i))) + i*SMALL_LOGIC_CAM_SIZE,
						write_pointer
					),
					pop_cam_match_addr'length
				));
...
--			end if;
			-- cam empty flag
			if (debug_msg2.push_cnt = (debug_msg2.push_cnt'range => '0') and
				debug_msg2.pop_cnt = (debug_msg2.pop_cnt'range => '0') and
				debug_msg2.overwrite_cnt = (debug_msg2.overwrite_cnt'range => '0')) then -- very clean, no underflow
	...
	begin 
		if (i_rst = '1') then 
			...
			case run_state_cmd is 
				...
				when TERMINATING => 
					if (read_time_ptr >= gts_end_of_run + to_unsigned(TERMINATION_DRAIN_GUARD_CYCLES_CONST, gts_end_of_run'length)) then -- TODO: change it to output fifo empty
						asi_ctrl_ready			<= '1';
					else
						asi_ctrl_ready			<= '0';
					end if;
