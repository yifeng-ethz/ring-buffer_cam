...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--signoff-effort", default="practical", choices=["practical", "extensive"])
    parser.add_argument("--isolated-stress-effort", default="extensive", choices=["practical", "extensive"])
...
def parse_doc_cases(doc_path: Path) -> list[dict[str, str]]:
    ...
    for line in doc_path.read_text().splitlines():
        ...
        if match:
            ...
            if case_id != "ID":
                cases.append({"case_id": case_id, "method": "D", "scenario": ""})
    return cases
...
def load_case_lists(ip_root: Path) -> OrderedDict[str, list[str]]:
    ...
    for bucket, filename in BUCKET_FILES.items():
        case_lists[bucket] = [record["case_id"] for record in parse_doc_cases(tb_root / filename)]
    return case_lists


def load_case_records(ip_root: Path) -> OrderedDict[str, list[dict[str, str]]]:
    tb_root = ip_root / "tb"
    case_records: OrderedDict[str, list[dict[str, str]]] = OrderedDict()
    for bucket, filename in BUCKET_FILES.items():
        case_records[bucket] = parse_doc_cases(tb_root / filename)
    return case_records


def needs_extensive_isolated(record: dict[str, str], bucket: str) -> bool:
...
def main() -> None:
    ...
    if not args.no_isolated_refresh:
      case_records = load_case_records(ip_root)
      isolated_cmd = [
          sys.executable,
      ...
      extensive_cases = [
          record["case_id"]
          for bucket, records in case_records.items()
          for record in records
          if needs_extensive_isolated(record, bucket)
      ]
      if extensive_cases:
          extensive_cmd = [
              sys.executable,
              ...
              "--refresh-matched",
              "--case-list",
              ",".join(extensive_cases),
              "--extra-plusargs",
              f"+FRCV_EFFORT={args.isolated_stress_effort} +FRCV_TIMEOUT_NS=20000000",
          ]
          run_cmd(extensive_cmd, ip_root)
    ...
    for key, build_cases in build_groups.items():
        ...
        for bucket, bucket_cases in case_lists.items():
            ...
            extra = (
                f"+FRCV_EXEC_MODE=bucket_frame +FRCV_EFFORT={args.signoff_effort} "
                f"+FRCV_CASE_LIST={case_list} +FRCV_SEQUENCE_NAME={bucket}_{build['build_tag']}"
            )
        ...
        extra = (
            f"+FRCV_EXEC_MODE=all_buckets_frame +FRCV_EFFORT={args.signoff_effort} "
            f"+FRCV_CASE_LIST={','.join(all_cases)} "
        ...
        if build["build_tag"] == "CFG_A":
            for cross_run_id, cross_extra in (
                (
                    ...
                    f"+FRCV_EXEC_MODE=cross +FRCV_EFFORT={args.signoff_effort} +FRCV_CROSS_PLAN=GOOD_ERROR_GOOD",
                ),
                (
                    "cross_interleave_mix_cfg_a",
                    f"+FRCV_EXEC_MODE=cross +FRCV_EFFORT={args.signoff_effort} +FRCV_CROSS_PLAN=INTERLEAVE_MIX",
                ),
                (
                    "cross_super_long_counter_soak_cfg_a",
                    "+FRCV_EXEC_MODE=cross +FRCV_EFFORT=extensive +FRCV_TIMEOUT_NS=20000000 +FRCV_CROSS_PLAN=SUPER_LONG_COUNTER_SOAK",
                ),
            ):
