...
def parse_signoff_banner(log_path: Path) -> dict[str, object]:
    ...
    if not log_path.exists():
        return payload
    list_re = re.compile(
        r"DOC_CASE_LIST_ENGINE_V1 mode=(\S+) build=(\S+) seq=(\S+) case_count=(\d+)(?:\s+iter_cap=(\d+)\s+payload_cap=(\d+))?"
    )
    cross_re = re.compile(r"FRCV_CROSS_ENGINE_V1 plan=(\S+) build=(\S+)")
    for line in log_path.read_text(errors="ignore").splitlines():
            ...
            payload.update(
                {
                    ...
                    "sequence_name": list_match.group(3),
                    "case_count": int(list_match.group(4)),
                    "iter_cap": int(list_match.group(5)) if list_match.group(5) else None,
                    "payload_cap": int(list_match.group(6)) if list_match.group(6) else None,
                }
            )
            continue
...
def main() -> None:
    ...
    for run in signoff_runs:
        ...
        if run["kind"] == "bucket_frame":
            signoff_bucket_rows.append(
                f"| {run['bucket']} | {run['build_tag']} | {int(run['case_count'])} | {format_cov(run['code_coverage'])} | {cross_pct} | {txn_count} | {overlap} |"
            )
        elif run["kind"] == "all_buckets_frame":
            signoff_all_rows.append(
                f"| {run['build_tag']} | {int(run['case_count'])} | {format_cov(run['code_coverage'])} | {cross_pct} | {txn_count} | {overlap} |"
            )
        elif run["kind"] == "cross":
            curve = cross.get("curve", "") if cross else ""
            signoff_cross_rows.append(
                f"| {run['sequence_name']} | {run['build_tag']} | {format_cov(run['code_coverage'])} | {cross_pct} | {txn_count} | {overlap} | {curve if curve else 'n/a'} |"
            )
    bucket_frame_status = (
    ...
    report_lines = [
        ...
        *report_summary_rows,
        ...
        "",
        "| bucket | build | case_count | code_coverage_total | functional_cross_total | txn_count | queued_overlap |",
        "|---|---|---:|---|---|---:|---:|",
        *(signoff_bucket_rows if signoff_bucket_rows else ["| pending | pending | 0 | n/a | n/a | 0 | 0 |"]),
        "",
        "| build | case_count | code_coverage_total | functional_cross_total | txn_count | queued_overlap |",
        "|---|---:|---|---|---:|---:|",
        *(signoff_all_rows if signoff_all_rows else ["| pending | 0 | n/a | n/a | 0 | 0 |"]),
        "",
    ]
    ...
    report_lines.extend(
        [
            "## CROSS Bucket",
            "",
            "| sequence | build | code_coverage_total | functional_cross_total | txn_count | queued_overlap | txn_growth_curve |",
            "|---|---|---|---|---:|---:|---|",
            *(signoff_cross_rows if signoff_cross_rows else ["| pending | pending | n/a | n/a | 0 | 0 | n/a |"]),
            "",
        ]
    )
    report_lines.extend(
