...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--refresh-matched", action="store_true")
    parser.add_argument("--case-list")
    parser.add_argument("--case-regex")
    parser.add_argument("--extra-plusargs", default="")
...
def run_case(
    ...
    if extra_plusargs:
        cmd.append(f"EXTRA_PLUSARGS={extra_plusargs}")
...
def main() -> None:
    ...
    if args.case_list:
        selected = {case_id.strip() for case_id in args.case_list.split(",") if case_id.strip()}
        cases = [case_id for case_id in cases if case_id in selected]
    if args.case_regex:
        case_re = re.compile(args.case_regex)
        cases = [case_id for case_id in cases if case_re.search(case_id)]
    ...
    if args.refresh_all or args.refresh_matched:
    ...
    for key, group_cases in build_groups.items():
        ...
        for case_id in group_cases:
            ...
            if not args.refresh_all and not args.refresh_matched and passed_case(uvm_dir, case_id, args.rtl_variant, args.seed):
                skipped += 1
