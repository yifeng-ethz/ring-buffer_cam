...
- `quartus_system/model/` is the durable three-tier model tree (`analytical/`, `tlm/`, `on_board/`). `quartus_system/board_test/` remains the place for live board runners, reports, and canonical test plans.
...
- Phase 4 closure is gated by `quartus_system/board_test/phase4/TEST_PLAN_BASIC.md`: the BASIC-like bucket must include TLM, RTL simulation, on-board, and cross-layer DISLIN evidence for its directed cases before Phase 4 is called closed.

## Phase 4 emulator + histogram verification workflow (TLM -> RTL SIM -> board)

Authoritative catalog: [`quartus_system/board_test/phase4/TEST_PLAN_BASIC.md`](quartus_system/board_test/phase4/TEST_PLAN_BASIC.md).
Closure contract: [`quartus_system/board_test/phase4/test_plan.md`](quartus_system/board_test/phase4/test_plan.md).
Canonical board playbook: [`quartus_system/board_test/doc/TEST_PLAN.md`](quartus_system/board_test/doc/TEST_PLAN.md).
...
1. The 8 x `emulator_mutrig_N` instances are reachable through `mm_bridge` and produce monotone `frame_count` / `event_count` (per `quartus_system/board_test/doc/TEST_PLAN.md` section 4.2).
...
`doc/Archive/ethhw_reordering.pdf` (extract `doc/Archive/` from `doc/Archive.zip` once to read the PDFs). Treat these as the architecturally correct shapes:
...
The Python TLM in [`quartus_system/board_test/phase4/scripts/phase4_queue_tlm.py`](quartus_system/board_test/phase4/scripts/phase4_queue_tlm.py) and the HDL TLM in [`quartus_system/board_test/phase4/tlm/phase4_latency_tlm.sv`](quartus_system/board_test/phase4/tlm/phase4_latency_tlm.sv) are calibrated to reproduce these shapes; [`quartus_system/board_test/phase4/MATH_REPORT.md`](quartus_system/board_test/phase4/MATH_REPORT.md) records the constants. Any change to the TLM that breaks agreement with the slides above must be reverted.

### TLM -> RTL SIM -> board, with DISLIN in-situ comparison

For every Phase 4 basic-bucket case (TPB000..TPB100 in `TEST_PLAN_BASIC.md`):

1. Run the TLM form via [`quartus_system/board_test/phase4/scripts/run_phase4_tlm.sh`](quartus_system/board_test/phase4/scripts/run_phase4_tlm.sh) and `scripts/phase4_queue_tlm.py`. The TLM is the architecturally correct shape; do not edit it to fit a measurement.
2. Run the RTL SIM form via [`quartus_system/tb_int/INT_fe_scifi_v3-2026-04-17/scripts/run_dp_e2e.sh`](quartus_system/tb_int/INT_fe_scifi_v3-2026-04-17/scripts/run_dp_e2e.sh) or [`run_dp_hist_rate_sweep.sh`](quartus_system/tb_int/INT_fe_scifi_v3-2026-04-17/scripts/run_dp_hist_rate_sweep.sh) with the case's `TB_DP_*` plusargs.
3. Run the on-board form via [`quartus_system/board_test/script/run_phase4_emulator.py`](quartus_system/board_test/script/run_phase4_emulator.py) and [`probe_phase4_stage_counters.py`](quartus_system/board_test/script/probe_phase4_stage_counters.py).
4. Render the in-situ DISLIN comparison via [`quartus_system/board_test/phase4/scripts/render_phase4_plots.sh`](quartus_system/board_test/phase4/scripts/render_phase4_plots.sh). The comparison must overlay all three layers (TLM + RTL SIM + BOARD) on one plot so disagreements are immediately visible.
...
Phase 4 cannot be marked closed until the pre-gate is signed (case TPBH030 PASS in `quartus_system/board_test/reports/phase4_overflow_pregate_<date>.md`).
