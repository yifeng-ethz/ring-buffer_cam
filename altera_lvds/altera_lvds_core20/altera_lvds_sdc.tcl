...
##########################################################################################
   ...
   append sdc_content "set corename altera_lvds_core20 \n"
   append sdc_content "\n"
...
if {$fit_flow} {
   ...
   if {[param_string_compare ENABLE_DIV_RECONFIG "true"]} {
      append sdc_content "set user_dprio_clk_multiply_by $m_cnt \n"
      append sdc_content "set user_dprio_clk_divide_by [expr $n_cnt * $c3_cnt] \n"
      append sdc_content "set user_dprio_clk_phase 0 \n"
   }
   ...
   set rccs_val [get_parameter_value RX_RCCS_VAL ]
   append sdc_content "\n   set ::RCCS(\$lvds_instance_name) $rccs_val \n"
   append sdc_content {   set half_RCCS [expr $::RCCS($lvds_instance_name) / 2.0] } 
...
proc generate_rx_fwd_clock { } {
   ...
   append sdc_content "if \{\[is_fitter_in_qhd_mode\]\} \{ \n"
   ...
   append sdc_content "\} else \{ \n"
	append sdc_content "   create_clock -name \$core_ck_name -period \$slow_clock_period_ns \${lvds_instance_name}|channels\[0\].soft_cdr.divfwdclk \n"
   append sdc_content "\} \n"
...
proc cut_paths {} {
    ...
    append sdc_content "set_false_path -to \$lvds_instance_name|channels\[*\].$lvds_if_name.serdes_dpa_inst*\n"
    append sdc_content "set_false_path -from \$lvds_instance_name|channels\[*\].$lvds_if_name.serdes_dpa_inst*\n"
...
proc generate_clocks {} {
   ...
   if {$pll_tx_outclock_lden_name != ""} {
      ...
      if {![param_string_compare MODE "TX"] || [param_string_compare TX_REGISTER_CLOCK "tx_coreclock"]} {
         if {[param_string_compare MODE "TX"]} {
            ...
            append sdc_content {   if {[catch {set pll_out_pin [get_node_info -name [altera_iosubsystem_get_src_pll_out_pin ${lvds_core_instance_name}|dprio_clk_gen.dprio_div_counter[1]]]}]} { set pll_out_pin "" } } \n
         }
   ...
   if {!$use_external_pll && $pll_ref_ck_name != "" && $pll_out_pin != ""} { 
      ...
      lappend core_clocks  "${pll_out_pin}" 
         ...
         if {[param_string_compare ENABLE_DIV_RECONFIG "true"]} {
            append sdc_content {   if {[catch {set user_dprio_clk_pin [get_node_info -name [altera_iosubsystem_get_src_pll_out_pin ${lvds_core_instance_name}|dprio_clk_gen.dprio_cycle_counter[0]]]}]} { set user_dprio_clk_pin "" } } \n
            append sdc_content {   
   if {!$use_external_pll && $pll_ref_ck_name != "" && $user_dprio_clk_pin != ""} { 
      altera_iosubsystem_create_generated_clock -source "${pll_ref_ck_name}" -divide_by $user_dprio_clk_divide_by -multiply_by $user_dprio_clk_multiply_by -phase $user_dprio_clk_phase -duty_cycle 50.00 -name "${user_dprio_clk_pin}" -target "${user_dprio_clk_pin}"  
      lappend core_clocks  "${user_dprio_clk_pin}" 
   } 
         } \n
         }
   ...
   set pll_out_pin_base [string range $pll_out_pin 0 [expr [string last {[} $pll_out_pin]-1] ] 
            ...
            for { set i $pll_maximum_number_of_reserved_clocks } {$i < $max_clocks} {incr i} {
               if {$i == $pll_maximum_number_of_reserved_clocks} {
                  append sdc_content "   if  \{!\$use_external_pll && \$pll_ref_ck_name != \"\" && \$pll_out_pin != \"\"\} \{ \n"
               }
...
proc generate_derive_pll_clocks {} {
   set sdc_content ""

   append sdc_content "\n"
   append sdc_content "if {\[altera_iosubsystem_get_a10_iopll_workaround_present\]} { \n"
...
proc generate_cut_path_for_internal_synchronizer {} {
   ...
   set list_of_func [list reset ctrl]
   
   foreach i_func $list_of_func {
      set i_to "\[get_keepers -nowarn \${lvds_core_instance_name}|channels\[*\].${block}.rx_bitslip|bitslip_${i_func}_synchronizer|sync_reg\[0\]\]"

      append sdc_content "\n"
      append sdc_content "   # Set max delay constraint in FIT to avoid rediculous long $i_func path but cut the timing path in STA since we have synchronizer \n"
      append sdc_content "   set to_pin_collection $i_to \n"
      append sdc_content "   if {\[get_collection_size \$to_pin_collection\] > 0} { \n"
      append sdc_content "      if {\$fit_flow == 1} { \n"
      append sdc_content "         set_max_delay -to $i_to \[expr 2*\$slow_clock_period_ns\] \n"
      append sdc_content "      } else { \n"
      append sdc_content "         set_false_path -to $i_to \n"
      append sdc_content "      } \n"
      append sdc_content "   } \n"
      append sdc_content "\n"
   }

   set reset_sync_reg {[get_pins -compatibility_mode -nowarn ${lvds_core_instance_name}|*pll_areset_sync*sync_reg[*]|clrn]}
   append sdc_content "\n"
...
proc generate_sdc_file { top_level } {
   ...
   append sdc_content [generate_derive_pll_clocks]
