...
proc altera_iosubsystem_is_node_pll_in_pin {node_id} {
	...
	if {$node_type == "pin" && $cell_wysiwyg_type == "fourteennm_iopll" && [get_pin_info -is_in_pin $node_id] == 1} {
		set result 1
	} else {
		set result 0
	}
...
proc altera_iosubsystem_is_node_pll_out_pin {node_id} {
	set node_type [get_node_info -type $node_id]
	set cell_wysiwyg_type [get_cell_info -wysiwyg_type [get_node_info -cell $node_id]]
	if {$node_type == "pin" && $cell_wysiwyg_type == "fourteennm_iopll" && [get_pin_info -is_out_pin $node_id] == 1} {
		set result 1
	} else {
		set result 0
	}
...
proc altera_iosubsystem_get_pll_inclock_name {lvds_instance_name} {
	...
	set pll_instance_name "$lvds_core_instance_name|internal_pll.pll_inst|fourteennm_pll" 
