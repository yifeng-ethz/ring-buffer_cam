#!/usr/bin/env python3
"""Generate a compact SignalTap file for FEB v3 reset-link ingress debug."""
...
DP = f"{QSYS}feb_system_v3_pipe_data_path_subsystem:data_path_subsystem|"
UP = f"{QSYS}feb_system_v3_pipe_upload_subsystem:upload_subsystem|"
HOST = f"{UP}runctl_mgmt_host:runctl_mgmt_host_0|"

CLOCK = f"{DP}lvds_outclock_clk"
...
def default_signals() -> list[str]:
    sigs: list[str] = []
    sigs += [
        f"{HOST}lvdspll_reset",
    ]

    sigs += bits(UP, "synclink_data", 9)
    sigs += bits(UP, "synclink_error", 3)
    sigs += bits(HOST, "asi_synclink_data", 9)
    sigs += bits(HOST, "asi_synclink_error", 3)

    sigs += [
        f"{HOST}synclink_idle_armed",
        f"{HOST}ev_cmd_accepted",
        f"{HOST}ev_rx_error",
        f"{HOST}local_cmd_pending_lvds",
        f"{HOST}local_cmd_consume_lvds",
    ]
    sigs += bits(HOST, "recv_run_command", 8)

    sigs += [
        f"{UP}runctl_mgmt_host_valid",
        f"{UP}runctl_mgmt_host_ready",
    ]
    sigs += bits(UP, "runctl_mgmt_host_data", 9)
    return sigs
...
def build_stp(sample_depth: int, trigger_signal: str, trigger_mode: str) -> ET.ElementTree:
    ...
    signal_set_name = "runctl_ingress_lvds"
    trigger_name = "rstlink_lane8_activity"
    ...
    ET.SubElement(signal_set, "clock", {"name": CLOCK, "polarity": "posedge", "tap_mode": "classic"})
    ET.SubElement(
        signal_set,
        "config",
        {
    ...
    if trigger_mode == "high":
        level.text = f"'{trigger_signal}' == high"
    elif trigger_mode == "rising_edge":
        level.text = f"'{trigger_signal}' == rising edge"
    elif trigger_mode == "falling_edge":
        level.text = f"'{trigger_signal}' == falling edge"
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "signaltap" / "phase3_runctl_ingress_lvds.stp",
    )
    parser.add_argument("--sample-depth", type=int, default=1024)
    parser.add_argument("--trigger-mode", choices=("high", "rising_edge", "falling_edge"), default="rising_edge")
    parser.add_argument(
        "--trigger-signal",
        default=f"{UP}synclink_data[8]",
    )
