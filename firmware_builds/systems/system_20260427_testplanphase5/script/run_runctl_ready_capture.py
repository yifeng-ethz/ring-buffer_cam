#!/usr/bin/env python3
"""Arm the run-control SignalTap capture, send one RC command, and summarize it."""
...
def default_stp() -> Path:
    return BOARD_TEST_DIR / "signaltap" / "phase4c_runctl_ready_fanout.stp"


def default_capture_tcl() -> Path:
    return SCRIPT_DIR / "capture_runctl_ready_stp.tcl"


def default_summary_py() -> Path:
    return SCRIPT_DIR / "summarize_runctl_ready_vcd.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=default_stp(),
        help="SignalTap .stp file to arm",
    )
    parser.add_argument(
        "--capture-tcl",
        type=Path,
    ...
    parser.add_argument(
        "--summary-py",
        type=Path,
        default=default_summary_py(),
        help="VCD summary helper",
    )
    parser.add_argument(
        "--rc-tool",
        type=Path,
    ...
    parser.add_argument(
        "--trigger-name",
        default="runctl_host_valid_rise",
        help="SignalTap trigger name to use",
    )
    parser.add_argument(
        "--out-vcd",
        type=Path,
        default=Path("/tmp/runctl_ready_fanout.vcd"),
        help="Output VCD path",
    )
    parser.add_argument(
        "--command",
        default="reset",
        help="rc_tool command to send after arming SignalTap",
    )
    parser.add_argument(
        "--device",
        default="/dev/mudaq0",
        help="rc_tool device path",
    )
    parser.add_argument(
        "--feb",
        type=int,
        default=7,
        help="rc_tool FEB ID; 7=broadcast",
    )
    parser.add_argument(
        "--run",
        type=int,
        default=42,
        help="Run number for commands that consume one",
    )
    parser.add_argument(
        "--settle-us",
        type=int,
        default=5000,
        help="rc_tool settle time",
    )
    parser.add_argument(
        "--arm-delay-s",
        type=float,
        default=2.0,
        help="Delay after arming SignalTap before issuing rc_tool",
    )
    parser.add_argument(
        "--capture-timeout-s",
        type=float,
        default=90.0,
        help="Timeout for the quartus_stp capture process",
    )
...
def main() -> int:
    ...
    summary_py = args.summary_py.resolve()
    out_vcd = args.out_vcd.resolve()

    for path in (stp_file, capture_tcl, summary_py, args.rc_tool):
    ...
    try:
        ...
        try:
            capture_stdout, _ = capture_proc.communicate(timeout=max(args.capture_timeout_s, 1.0))
        ...
        summary = run_checked([sys.executable, str(summary_py), str(out_vcd)])
        print("summary:")
