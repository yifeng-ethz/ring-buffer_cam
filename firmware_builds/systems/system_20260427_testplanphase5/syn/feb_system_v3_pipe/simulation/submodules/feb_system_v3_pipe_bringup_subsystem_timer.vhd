...
entity feb_system_v3_pipe_bringup_subsystem_timer is 
        port (
              -- inputs:
...
end entity feb_system_v3_pipe_bringup_subsystem_timer;


architecture europa of feb_system_v3_pipe_bringup_subsystem_timer is
                ...
                signal counter_load_value :  STD_LOGIC_VECTOR (15 DOWNTO 0);
...
begin
  ...
  begin
    if reset_n = '0' then
      internal_counter <= std_logic_vector'("1100001101001111");
    elsif clk'event and clk = '1' then
      if std_logic'((counter_is_running OR force_reload)) = '1' then 
        if std_logic'((counter_is_zero OR force_reload)) = '1' then 
          internal_counter <= counter_load_value;
        else
          internal_counter <= A_EXT (((std_logic_vector'("00000000000000000") & (internal_counter)) - std_logic_vector'("000000000000000000000000000000001")), 16);
        end if;
      end if;
    end if;
  ...
  counter_is_zero <= to_std_logic(((std_logic_vector'("0000000000000000") & (internal_counter)) = std_logic_vector'("00000000000000000000000000000000")));
  counter_load_value <= std_logic_vector'("1100001101001111");
  process (clk, reset_n)
  begin
    if reset_n = '0' then
