...
set_module_property VERSION 26.1.8.0419
set_module_property DESCRIPTION "Ring-buffer shaped content-addressable memory for timestamp ordering. This delivered release keeps the current Qsys interface contract, adds the common CSR identity header, keeps the verified long-run counter cleanup and same-key overwrite-tail fixes, and packages the refreshed PROF closure with unique long-run fingerprints plus explicit seeded multi-key evidence."
...
add_html_text "Delivered Profile" profile_html "<html><b>Catalog revision</b><br/>This release is packaged as <b>26.1.8.0419</b>. It keeps the established <b>hit_type1</b>, <b>hit_type2</b>, <b>run_control</b>, and <b>filllevel</b> interface names so existing Platform Designer systems can be upgraded in place while picking up the common CSR identity header, the locked `BUILD=419` / `VERSION_DATE=20260419` metadata, and the refreshed PROF closure with unique long-run fingerprints, calibrated deterministic multi-key integrity, and explicit seeded random evidence.<br/><br/><b>Runtime visibility</b><br/>Software can blind-scan the CSR window through <b>UID</b> at word <b>0</b> and the <b>META</b> mux at word <b>1</b>.</html>"
