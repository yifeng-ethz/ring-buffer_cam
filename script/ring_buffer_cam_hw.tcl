...
set_module_property VERSION 26.1.9.0419
set_module_property DESCRIPTION "Ring-buffer shaped content-addressable memory for timestamp ordering. This delivered release keeps the current Qsys interface contract, adds the common CSR identity header, keeps the verified long-run counter cleanup and same-key overwrite-tail fixes, and packages the refreshed PROF closure with calibrated steady-state, adversarial overlap, and partition-handoff evidence."
